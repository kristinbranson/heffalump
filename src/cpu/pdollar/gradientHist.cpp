...
*
* Modifiedfrom gradentMex.cpp by Nathan Clack 2017
* - removing mex interface
* - focus implementation on gradentHist
*******************************************************************************/
...
Compared to pdollar's hog(), gradHist is just the basics: no normalization
and no locally normalized features.
...
extern "C" void gradHist( float *M, float *O, float *H, int h, int w,
  int bin, int nOrients, int softBin, bool full )
{
  ...
  for( x=0; x<w0; x++ ) {
    ...
    if( softBin<0 && softBin%2==0 ) { // negative and even
    ...
    } else if( softBin%2==0 || bin==1 ) { // odd or cell size (bin) is 1 px
    ...
    } else { // cell size (bin) > 1 and softBin is positive and even
  ...
  // Kicks in when softBin is odd.
  //
  // TODO: (ngc) understand this
